...
void Jobisjob::extract_offers (GumboNode * node)
{
      ...
      (attributes2 = gumbo_get_attribute(&node->parent->v.element.attributes, "class")))
  {
        ...
        strstr(attributes2->value, "box_offer") != NULL)
    {
      ...
      if (title.find("Vendeur") != std::string::npos || title.find("vendeur") != std::string::npos ||
          title.find("Livreur") != std::string::npos || title.find("livreur") != std::string::npos ||
	  title.find("Chauffeur") != std::string::npos || title.find("chauffeur") != std::string::npos){
        tab_offer.pop_back();
      }

      string description = this->tab_offer[num_offer]->get_description();
      if (description.find("- Règles d'hygiène et de sécurité alimentaire- Procédure d'encaissement- Utilisation de trancheuse- Calcul de quantité") != std::string::npos ||
          description.find("- Procédures de fabrication de produits de boulangerie- Éléments de base en pâtisserie") != std::string::npos)
      {
        tab_offer.pop_back();
      }
    }
  ...
  {  
        ...
        strstr(attributes2->value, "address") != NULL)
    {
      ...
      if (child->v.text.text != NULL )
      {
        ...
        std::size_t pos = location.find(", Aquitaine Limousin Poitou-Charentes");
      ...
      (attributes = gumbo_get_attribute(&node->parent->parent->parent->v.element.attributes, "class")))
  {
    if (strstr(attributes->value, "paginator") != NULL)
    {
      ...
      if (child->v.element.tag != GUMBO_TAG_STRONG) // if it's not the "next page"
      {
        ...
        std::size_t pos1 = value.find("/emploi");
        std::size_t pos2 = value.find("?", pos1);
        std::size_t pos3 = this->current_url.find("/emploi");
